...
export default class TransactionScreen extends Component {
  ...
  checkStudentEligibilityForBookIssue = async (studentId) =>{
    ...
    if (studentRef.docs.length == 0) {
      ...
      this.setState ({bookId:"", studentId:""})
      Alert.alert("Este estudiante no existe");
    } else {
      studentRef.docs.map(doc => {
        if(doc.data().number_of_books_issued>=4){
          Alert.alert("Ya no puedes sacar mas libros");
          this.setState({bookId:"", studentId:""})
          isStudentEligible=false
        } else{
...
checkStudentEligibilityForBookReturn = async (bookId, studentId) => {
  const transactionRef = await db
    ...
    .get();
  var isStudentEligible = "";
  transactionRef.docs.forEach(doc => {
 ...
 //ToastAndroid.show("El libro no fue emitido a este alumno", ToastAndroid.SHORT);
  ...
  // getBookDetails = async (studentId) => {
  //   try {
  //   studentId = studentId.trim();
  //   const snapshot = await db.collection("students").where("student_id", "==", studentId).get();
  //   snapshot.docs.forEach(doc => {
  //   this.setState({
      ...
      <KeyboardAvoidingView behavior="padding" style={styles.container}>
        <ImageBackground source={bgImage} style={styles.bgImage}>
          ...
          <View style={styles.textinputContainer}>
            <TextInput style={styles.textinput}placeholder={"Id del libro"}placeholderTextColor={"#FFFFFF"}value={bookId}
